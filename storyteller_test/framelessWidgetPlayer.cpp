...
#include "ui_framelessWidgetPlayer.h"

framelessWidgetPlayer::framelessWidgetPlayer(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::framelessWidgetPlayer)
{
    ui->setupUi(this);
...
framelessWidgetPlayer::~framelessWidgetPlayer()
{
    delete ui;
}

/* 给centralwidget添加一个mainwidget，设置遮罩及遮挡锯齿边缘的board*/
void framelessWidgetPlayer::Init()
{
...
void framelessWidgetPlayer::mousePressEvent(QMouseEvent *event)
{
...
// 鼠标移动事件
void framelessWidgetPlayer::mouseMoveEvent(QMouseEvent *event)
{
...
// 窗口变化事件
void framelessWidgetPlayer::resizeEvent(QResizeEvent *event)
{
    //Resize border
    if(border)
        border->resize(ui->mainWidget->size() + QSize(2, 2));
...
//点击最大化按钮事件
void framelessWidgetPlayer::controlWindowScale(){
#ifdef Q_OS_WINDOWS
    if(!maximized){
