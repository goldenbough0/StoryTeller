...
StoryNode::StoryNode(qreal x, qreal y, qreal width, qreal height, QGraphicsItem *parent) : QGraphicsEllipseItem(x, y, width, height, parent)
{
    // 设置节点可移动和可选择
...
QVariant StoryNode::itemChange(GraphicsItemChange change, const QVariant &value)
{
...
// 鼠标按下事件
void StoryNode::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    // 调用父类的鼠标按下事件
...
// 鼠标移动事件
void StoryNode::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    // 调用父类的鼠标移动事件
...
// 获取节点是否被选中
bool StoryNode::isSelected() const
{
    return QGraphicsEllipseItem::isSelected();
}

// 更新连接线的位置
void StoryNode::updateConnectLines()
{
